...
class CommentClassification {
	...
	public static float calcInterestingLevelProgrammers(comment) {
		float matchPattern = bodyContainsProgrammersLink(comment.body) ? 1.0f : 0f;
		return matchPattern + calcInterestingLevelProgrammers(comment.body_markdown);
	}
	
	public static float calcInterestingLevelSoftwareRecs(comment) {
		return calcInterestingLevelSoftwareRecs(comment.body_markdown);
	}
...
//		points -= score(0.25f, comment, "meta.softwarerecs.stackexchange.com/q/336/");
	...
	private static final String[] programmersIgnore = [ "please stop using programmers.se as your toilet bowl",
