...
class StackExchangeLogin(private val httpClient: HttpClient, private val config: BotConfig) {
    ...
    suspend fun login(): Boolean {
        val loginPage: String = httpClient.get("${config.rootUrl}/users/login").body()
        val fkey = Jsoup.parse(loginPage).selectFirst("input[name='fkey']")!!.attr("value")
        println(fkey)

        val r = httpClient.submitForm(config.rootUrl + "/users/login", Parameters.build {
        ...
        val currentUserHtml: String = httpClient.get(config.rootUrl + "/users/current").body()
    ...
    suspend fun fkeyReal(): String {
        val favoriteChatsHtml: String = httpClient.get(config.chatUrl + "/chats/join/favorite").body()
